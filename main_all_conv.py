...
tf.app.flags.DEFINE_integer('hidden_size', 3,
  ...
  with tf.Graph().as_default():
    ...
    for step in xrange(FLAGS.max_step):
      ...
      if step%500 == 0:
        ...
        #kl_loss_dis = np.sort(np.sum(kl_loss_dis.reshape(FLAGS.batch_size, 32), axis=0))
        stddev_r = np.sort(np.sum(stddev_r.reshape(FLAGS.batch_size, 16*FLAGS.hidden_size), axis=0))
        print(stddev_r)
